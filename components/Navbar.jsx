...
export default function Navbar({ activePage = 'Ana Səhifə' }) {
  ...
  return (
    <nav style={{ background: 'transparent', position: 'sticky', top: 0, zIndex: 100, width: '100%' }}>
      <style>{`
        .nav-inner { padding: 12px 40px 0; }
        .nav-card { padding: 0 28px; }
        .desktop-nav { display: flex; }
        .mobile-nav-btn { display: none; }
        @media (max-width: 900px) {
          .nav-inner { padding: 8px 16px 0; }
          .nav-card { padding: 0 16px; height: 56px !important; }
      ...
      <div className="nav-inner">
        <div className="nav-card" style={{ maxWidth: '1440px', margin: '0 auto', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(20,20,26,0.7)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', boxSizing: 'border-box' }}>
          <Link href="/" style={{ textDecoration: 'none', flexShrink: 0 }}>
            <img className="nav-logo" src="/logo.png" alt="Changers Academy" style={{ height: '36px', width: '110px', objectFit: 'contain' }} />
          </Link>
