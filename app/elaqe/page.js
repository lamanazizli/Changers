    ...
    <main style={{ background: '#0B0B0F', minHeight: '100vh' }}>
      ...
      <style>{`
        .elaqe-hero-glow {
          ...
          background: radial-gradient(circle, rgba(255,44,168,0.15) 0%, transparent 70%);
          pointer-events: none;
        }
        .elaqe-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        .faq-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; }
        @media (max-width: 640px) {
          .elaqe-form-grid { grid-template-columns: 1fr; }
          .faq-grid { grid-template-columns: 1fr; }
        }
      `}</style>

      {/* Hero */}
      <section className="section" style={{ position: 'relative', paddingBottom: '48px', overflow: 'hidden' }}>
        <div className="elaqe-hero-glow" />
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h1 className="h1" style={{ color: '#FFFFFF', marginBottom: '16px' }}>
            Sualın var?<br /><span style={{ color: '#FF2CA8' }}>Bizə yaz!</span>
          </h1>
          <p className="text-body" style={{ color: '#A0A0B0', margin: 0, maxWidth: '500px' }}>
      ...
      <section className="section" style={{ paddingTop: 0 }}>
        <div className="container grid-2" style={{ alignItems: 'flex-start' }}>
          ...
          {/* Left — Contact Info */}
          <div>
            <h2 className="h3" style={{ color: '#FFFFFF', marginBottom: '32px', fontSize: '28px' }}>Əlaqə məlumatları</h2>
          ...
          <div>
            {sent ? (
              <div style={{ background: 'rgba(0,214,143,0.08)', border: '1px solid rgba(0,214,143,0.3)', borderRadius: '20px', padding: '48px 24px', textAlign: 'center' }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,44,168,0.2)', borderRadius: '20px', padding: '24px' }}>
                <h2 style={{ fontWeight: 700, fontSize: '24px', color: '#FFFFFF', margin: '0 0 28px 0' }}>Mesaj göndər</h2>
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div className="elaqe-form-grid">
                    <div>
                  ...
                  <button type="submit" disabled={loading} style={{ background: '#FF2CA8', color: '#FFFFFF', border: 'none', borderRadius: '10px', padding: '16px', fontSize: '15px', fontWeight: 700, cursor: 'pointer', marginTop: '8px', opacity: loading ? 0.7 : 1 }}>
